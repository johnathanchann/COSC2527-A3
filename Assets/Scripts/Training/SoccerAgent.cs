...
public class SoccerAgent : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        if (behaviorParams.BrainParameters.VectorObservationSize == 0)
        {
            return;
        }
        sensor.AddObservation(GetPlayerVelocity());
        sensor.AddObservation(GetBallVelocity());
        sensor.AddObservation(GetOpponentVelocity());
    }
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        //float velocityTowardBall = Vector2.Dot(_playerRb.linearVelocity.normalized, toBall) * _playerRb.linearVelocity.magnitude;
        //AddReward(velocityTowardBall * 0.05f);
        //print($"[{_player.name}] Reward for velocity toward ball: {velocityTowardBall * 0.05f}");

        // == Reward 3: Forward velocity ==
        //AddReward(_playerRb.linearVelocity.magnitude * 0.1f);
    ...
    public void ChangeSide(Team team)
    {
        GetComponent<Player>().team = team;
        _sign = team == Team.Left ? 1f : -1f;
        // Get all children transforms
        Transform[] childTransforms = GetComponentsInChildren<Transform>();
        // Skip the parent transform (this gameObject's transform)
        foreach (Transform childTransform in childTransforms)
        {
            if (childTransform != transform)
            {
                childTransform.rotation = Quaternion.Euler(0, 0, -90f * _sign);
            }
        }
    }
}
